...
#   Combinatoric tool
    ...
    def shapleyLorenz_val(self, X, y, class_prob = False, pred_out = 'predict', **kwargs):
       ...
       #Conditions on y
        ...
        for i,s in enumerate(self.powerset(range(self.M-1))):
        ...
        for k in tqdm(range(self.M)):
            ...
            Lor_k = np.zeros((2**(self.M-1),1))
            Lor_base = np.zeros((2**(self.M-1),1))
            ...
            if class_prob == False or (class_prob == True and pred_out == 'predict'):
                ...
                y_k = np.ones((self.N_test, 2**(self.M-1)))
            ...
            self.LZ[k,0] = np.dot(Lor_val.T,self.kernel) # equation 19 on page 10 of Giudiuci and Raffinetti (Feb 2020) paper
...
dir = os.getcwd()
df = pd.read_excel(os.path.join(dir,'final_dataset_smes.xlsx'), sheet_name = 'final_dataset_smes')
features = pd.read_excel(os.path.join(dir,'final_dataset_smes.xlsx'), sheet_name = 'Sheet1')
feat_labels = features.iloc[:,1]
feat_labels_short = features.iloc[:,0]
feat_to_drop = ['V_16', 'V_17', 'V_18', 'V_19', 'V_24']
feat_labels_short = [x for x in feat_labels_short if x not in feat_to_drop]
df = df.drop(feat_to_drop,1)
X = df.iloc[:,1:]
y = df.iloc[:,0]
X_sampled = X.sample(1000)
y_sampled = y[X_sampled.index]
#selected_feat = [23, 11, 15]
#X = df.iloc[:,selected_feat]
#y = df.iloc[:,0]
#df_new = df.iloc[:,selected_feat]

logReg_rII = sm.Logit(y,X).fit()

X_bg = X.sample(30)
y_bg = y[X_bg.index]

slz = ShapleyLorenzShare(logReg_rII.predict, X_bg, y_bg)
slz_values = slz.shapleyLorenz_val(X_sampled,y_sampled)

# logReg_r = sfm.logit('y~X', data = df).fit()
# logReg_r.summary()

slz_val_df = pd.DataFrame(slz_values)
with pd.ExcelWriter('slz_values.xlsx') as writer:
    slz_val_df.to_excel(writer, sheet_name = 'Shapley Lorenz Zonoid values')
os.getcwd()